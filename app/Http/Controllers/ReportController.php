    ...
    public function taxSummary(Request $request)
    {
        ...
        if(
            ...
            $invoiceProducts = InvoiceProduct::selectRaw('invoice_products.* ,MONTH(invoice_products.created_at) as month,YEAR(invoice_products.created_at) as year')
                ...
                ->whereRaw('YEAR(invoice_products.created_at) =?', [$year])->get();
            ...
            $billProducts = BillProduct::selectRaw('bill_products.* ,MONTH(bill_products.created_at) as month,YEAR(bill_products.created_at) as year')
                ->leftjoin('product_services', 'bill_products.product_id', '=', 'product_services.id')
                ->whereRaw('YEAR(bill_products.created_at) =?', [$year])->get();
...
//            -----------------------------
    ...
    public function salesReportExport(Request $request)
    {
        ...
        if ($request->report == '#item') {
            $invoiceItems = InvoiceProduct::select('product_services.itemNm', \DB::raw('sum(invoice_products.quantity) as quantity'), \DB::raw('sum(invoice_products.price * invoice_products.quantity) as price'), \DB::raw('sum(invoice_products.price)/sum(invoice_products.quantity) as avg_price'));
    ...
    public function ReceivablesReport(Request $request)
    {
        ...
        $receivableDetailsInvoice = Invoice::select('customers.name')
            ...
            ->selectRaw('(product_services.itemNm) as product_name')
            ->selectRaw('invoices.issue_date as issue_date')
            ->selectRaw('invoices.status as status')
            ->leftJoin('customers', 'customers.id', 'invoices.customer_id')
            ->leftJoin('invoice_products', 'invoice_products.invoice_id', 'invoices.id')
            ...
            ->where('invoices.issue_date', '>=', $start)
            ->where('invoices.issue_date', '<=', $end)
            ->groupBy('invoices.invoice_id', 'product_services.itemNm')
            ->get()
            ->toArray();
        ...
        $receivableDetailsCredit = CreditNote::select('customers.name')
            ...
            ->where('credit_notes.creditNoteDate', '>=', $start)
            ->where('credit_notes.creditNoteDate', '<=', $end)
            ->groupBy('credit_notes.id', 'product_services.itemNm')
            ->get()
            ->toArray();
    ...
    public function PayablesReport(Request $request)
    {
        ...
        $payableDetailsBill = Bill::select('venders.spplrNm')
            ...
            ->where('bills.bill_date', '>=', $start)
            ->where('bills.bill_date', '<=', $end)
            ->groupBy('bills.bill_id', 'product_services.itemNm')
            ->get()
            ->toArray();
        ...
        $payableDetailsDebit = DebitNote::select('venders.spplrNm')
            ...
            ->where('debit_notes.date', '>=', $start)
            ->where('debit_notes.date', '<=', $end)
            ->groupBy('debit_notes.id', 'product_services.itemNm')
            ->get()
            ->toArray();
